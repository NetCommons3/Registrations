 ...
 */
	public function export() {
		...
		try {
			...
			$this->_setFlashMessageAndRedirect(
				__d('registrations', 'export error' . $e->getMessage()));
			return;
		}
		// 大外枠zipファイル準備
		...
		$zipWrapperFile->addFromString(
			RegistrationsComponent::REGISTRATION_FINGER_PRINT_FILENAME,
			$fingerPrint);
		// 本体ファイルを
		$zipWrapperFile->addFile(
			$zipFile->path,
			RegistrationsComponent::REGISTRATION_TEMPLATE_FILENAME);
		// export-key 設定
