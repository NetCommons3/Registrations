<?php
/**
 * RegistrationQuestion::saveRegistrationQuestion()のテスト
 *
 ...
 */
class RegistrationSaveRegistrationQuestionTest extends RegistrationsSaveTest {
 ...
 * @var array
 */
	protected $_methodName = 'saveRegistrationQuestion';
 ...
 */
	private function __getData($key = null, $questionType = '1') {
		$data = array();
		$data['RegistrationQuestion'][0] = array(
			'language_id' => '2',
			'question_sequence' => '0',
			...
			'registration_page_id' => '54',	// 編集長が編集中のページデータのIDです
		);
		if (! is_null($key)) {
			$data['RegistrationQuestion'][0]['key'] = $key;
		}
		return $data;
	}
 ...
 */
	private function __getDataWithChoice($questionType = '1') {
		...
		$data['RegistrationQuestion'][0]['RegistrationChoice'] = $choices;
		return $data;
	}
/**
 * SaveのDataProvider
 ...
 */
	public function dataProviderSaveOnExceptionError() {
		$data = $this->__getData();
		return array(
			array($data, 'Registrations.RegistrationQuestion', 'save'),
		);
	}
/**
 ...
 */
	public function dataProviderSaveOnValidationError() {
		...
		return array(
			array($data['RegistrationQuestion'][0], $options, 'Registrations.RegistrationQuestion'),
		);
	}
/**
	...
	public function dataProviderValidationError() {
		...
		return array(
			...
			array($this->__getDataWithChoice(), $options, 'min', '',
				__d('registrations', 'Please enter both the maximum and minimum values.'), array('question_type' => 3, 'is_range' => true, 'max' => 8)),
		);
	}
