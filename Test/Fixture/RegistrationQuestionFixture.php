...
/**
 * Summary for RegistrationQuestionFixture
 */
class RegistrationQuestionFixture extends CakeTestFixture {
...
'),
		...
		'is_choice_horizon' => array('type' => 'boolean', 'null' => false, 'default' => '0', 'comment' => '選択肢横並び化 | 項目タイプが1:択一選択 2:複数選択 6:マトリクス（択一） 7:マトリクス（複数） のとき有効 ただし、６，７については行がランダムになるだけで列はランダム化されない'),
 ...
 */
	public function init() {
		...
		for ($registrationId = 1; $registrationId <= 50; $registrationId = $registrationId + 2) {
			$repeat = 1;
			if (in_array($registrationId, $threePages)) {
				$repeat = 3;
			} elseif (in_array($registrationId, $sevenPages)) {
				$repeat = 7;
			}
			for ($ii = 0; $ii < $repeat; $ii++) {
				$this->records[] = $this->getQuestions($pId, $registrationId, 1, $ii);
				$this->records[] = $this->getQuestions($pId + 1, $registrationId + 1, 2, $ii);
				$pId = $pId + 2;
			}
		}
		parent::init();
 ...
 * @return array
 */
	public function getQuestions($pId, $registrationId, $langId, $pageSeq) {
		$ret = array(
			'id' => $this->questionId,
			...
			'registration_page_id' => $pId,
			'created_user' => $this->getCreatedUser($registrationId),
			'created' => '2016-01-05 09:00:00',
			'modified_user' => $this->getCreatedUser($registrationId),
