...
?>
<div class="form-horizontal" collapse="createOption != '<?php echo RegistrationsComponent::REGISTRATION_CREATE_OPT_REUSE; ?>'">
	...
	<div class="form-group">
		...
		<ul class="col-xs-12 registration-select-box form-control ">
			<li class="animate-repeat btn-default"
				...
				btn-radio="item.registration.id" uncheckable>
				...
				<span ng-if="item.registration.publicType == <?php echo WorkflowBehavior::PUBLIC_TYPE_LIMITED; ?>">
				(
					{{item.registration.publishStart | ncDatetime}}
					<?php echo __d('registrations', ' - '); ?>
					{{item.registration.publishEnd | ncDatetime}}
