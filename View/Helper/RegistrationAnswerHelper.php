 ...
 */
	public function answer($question, $readonly = false) {
		...
		if (! RegistrationsComponent::isMatrixInputType($question['question_type'])) {
			...
			$ret .= $this->NetCommonsForm->hidden('RegistrationAnswer.' . $index . '.0.registration_answer_summary_id');
			$ret .= $this->NetCommonsForm->hidden('RegistrationAnswer.' . $index . '.0.registration_question_key', array('value' => $index));
			$ret .= $this->NetCommonsForm->hidden('RegistrationAnswer.' . $index . '.0.id');
			$ret .= $this->NetCommonsForm->hidden('RegistrationAnswer.' . $index . '.0.matrix_choice_key', array('value' => null));
		}
		return $ret;
	}
/**
 ...
 */
	public function singleChoice($index, $fieldName, $question, $readonly) {
		...
		if (isset($question['RegistrationChoice'])) {
			...
			if (Hash::extract($question['RegistrationChoice'], '{n}[other_choice_type!=' . RegistrationsComponent::OTHER_CHOICE_TYPE_NO_OTHER_FILED . ']')) {
	...
	public function singleText($index, $fieldName, $question, $readonly) {
		...
		if ($question['is_range'] == RegistrationsComponent::USES_USE) {
			...
			if ($question['question_type_option'] == RegistrationsComponent::TYPE_OPTION_NUMERIC) {
				$ret .= sprintf(__d('registrations', 'Please enter a number between %s and %s'), $question['min'], $question['max']);
			} else {
				$ret .= sprintf(__d('registrations', 'Please enter between %s letters and %s letters'), $question['min'], $question['max']);
			}
			$ret .= '</span>';
		}
		return $ret;
 ...
 */
	public function matrix($index, $fieldName, $question, $readonly) {
		if (isset($question['RegistrationChoice'])) {
			...
			$rowChoices = Hash::extract($question['RegistrationChoice'], '{n}[matrix_type!=' . RegistrationsComponent::MATRIX_TYPE_COLUMN . ']');
		...
		$ret = '<table class="table ' . $addClass . 'table-bordered text-center registration-matrix-table">';
		$ret .= '<thead><tr><th></th>';
		...
		foreach ($rowChoices as $rowIndex => $row) {
			...
			$ret .= $this->NetCommonsForm->hidden('RegistrationAnswer.' . $index . '.' . $rowIndex . '.registration_answer_summary_id');
			$ret .= $this->NetCommonsForm->hidden('RegistrationAnswer.' . $index . '.' . $rowIndex . '.registration_question_key', array('value' => $index));
			$ret .= $this->NetCommonsForm->hidden('RegistrationAnswer.' . $index . '.' . $rowIndex . '.matrix_choice_key', array('value' => $row['key']));
			$ret .= $this->NetCommonsForm->hidden('RegistrationAnswer.' . $index . '.' . $rowIndex . '.id');
			if ($row['other_choice_type'] != RegistrationsComponent::OTHER_CHOICE_TYPE_NO_OTHER_FILED) {
				$ret .= $this->NetCommonsForm->input('RegistrationAnswer.' . $index . '.' . $rowIndex . '.other_answer_value', array(
					'type' => 'text',
					'label' => false,
					'div' => false,
					'disabled' => $readonly,
			...
			$ret .= '</th>';
			$ret .= $this->_getMatrixRow($question['question_type'], 'RegistrationAnswer.' . $index . '.' . $rowIndex . '.answer_value', $options, $readonly);
			$ret .= '</tr>';
			$errorMessage .= $this->_error('RegistrationAnswer.' . $index . '.' . $rowIndex . '.answer_value');
		}
		$ret .= '</tbody></table>';
		$ret .= $errorMessage;
		return $ret;
	...
	public function dateTimeInput($index, $fieldName, $question, $readonly) {
		...
		if ($question['question_type_option'] == RegistrationsComponent::TYPE_OPTION_DATE) {
			...
			if ($question['is_range'] == RegistrationsComponent::USES_USE) {
				...
				$rangeMessage .= sprintf(__d('registrations', 'Please enter at %s to %s'), date('Y-m-d', strtotime($question['min'])), date('Y-m-d', strtotime($question['max'])));
			}
			...
			if ($question['is_range'] == RegistrationsComponent::USES_USE) {
				...
				$rangeMessage .= sprintf(__d('registrations', 'Please enter at %s to %s'), date('H:i', strtotime($question['min'])), date('H:i', strtotime($question['max'])));
			}
			...
			if ($question['is_range'] == RegistrationsComponent::USES_USE) {
				$options['minDate'] = $question['min'];
				$options['maxDate'] = $question['max'];
				$rangeMessage .= sprintf(__d('registrations', 'Please enter at %s to %s'), date('Y-m-d H:i', strtotime($question['min'])), date('Y-m-d H:i', strtotime($question['max'])));
			}
		}
 ...
 */
	protected function _getChoiceOptionElement($choices) {
		...
		foreach ($choices as $choice) {
			$ret[RegistrationsComponent::ANSWER_DELIMITER . $choice['key'] . RegistrationsComponent::ANSWER_VALUE_DELIMITER . $choice['choice_label']] = $choice['choice_label'];
		}
		return $ret;
	}
/**
