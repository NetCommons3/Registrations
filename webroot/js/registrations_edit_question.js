    ...
    function($scope, NetCommonsBase, NetCommonsWysiwyg,
      ...
      $scope.initialize =
        ...
        for (var pIdx = 0; pIdx <
          ...
          $scope.registration.registrationPage[pIdx].tabActive = false;
          ...
          // 各質問処理
          for (var qIdx = 0;
              qIdx < page.registrationQuestion.length;
              qIdx++) {
            ...
            for (var cIdx = 0;
                cIdx < question.registrationChoice.length;
                cIdx++) {
      ...
      $scope.changeDatetimepickerType = function(pIdx, qIdx) {
        var type = $scope.registration.registrationPage[pIdx].
            registrationQuestion[qIdx].questionTypeOption;
        var format;
        ...
        $scope.registration.registrationPage[pIdx].
            registrationQuestion[qIdx].min =
            $scope.getDateStr($scope.registration.registrationPage[pIdx].
                registrationQuestion[qIdx].min, format);
        $scope.registration.registrationPage[pIdx].
            registrationQuestion[qIdx].max =
            $scope.getDateStr($scope.registration.registrationPage[pIdx].
                registrationQuestion[qIdx].max, format);
      };
      ...
      $scope.addPage = function($event) {
        ...
        $scope.registration.registrationPage[$scope.registration.
            registrationPage.length - 1].tabActive = true;
        if ($event) {
      ...
      $scope._resetRegistrationPageSequence = function() {
        for (var i = 0;
            i < $scope.registration.registrationPage.length; i++) {
      ...
      $scope.addChoice =
        ...
        var page = $scope.registration.
            registrationPage[pIdx];
      ...
      $scope.changeSkipPageChoice = function(skipPageIndex) {
        ...
        if ($scope.registration.registrationPage.length - 1 >=
            skipPageIndex) {
          return;
        }
        // ないページを指定された場合は新しく作る
      ...
      $scope._resetRegistrationChoiceSequence = function(pageIndex, qIndex) {
        for (var i = 0; i <
            $scope.registration.registrationPage[pageIndex].
            registrationQuestion[qIndex].registrationChoice.length; i++) {
          $scope.registration.registrationPage[pageIndex].
